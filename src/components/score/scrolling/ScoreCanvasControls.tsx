...
export const ScoreCanvasControls: React.FC<ScoreControlsProps> = ({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5 bg-slate-900/90 border-b border-white/10 text-white select-none">
      ...
      {/* 3. Toggles de Exibição, Metrônomo e Tema */}
      <div className="flex items-center gap-2">
        {/* Metrônomo Sonoro */}
        <button
          onClick={onToggleMetronome}
          title={enableMetronome ? 'Metrônomo ligado' : 'Metrônomo mudo'}
        ...
        >
          {enableMetronome ? <Volume2 className="w-3.5 h-3.5" /> : <VolumeX className="w-3.5 h-3.5" />}
        </button>
