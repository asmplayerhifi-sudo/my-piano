...
type TabType = 'metronome' | 'accompaniment' | 'mixer';

export const MetronomeAccompanimentModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="bg-[#0b0f19] border border-white/10 rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        {/* 1. Header do Modal com Tabs e Botão Fechar */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 sm:p-5 border-b border-white/10 bg-gradient-to-r from-slate-900/80 via-indigo-950/40 to-slate-900/80">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center text-indigo-400 shadow-lg shadow-indigo-600/20">
              <Radio className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base sm:text-lg font-black text-white font-display">
                  Metrônomo &amp; Acompanhamento Musical
                </h2>
              ...
              <p className="text-xs text-slate-400">
                Sincronismo de alta precisão Web Audio com bateria, baixo, harmonia e arpejos.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto">
            {/* Navegação por Abas */}
            <div className="flex bg-black/50 p-1 rounded-2xl border border-white/5 text-xs font-bold">
              <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar">
          ...
          {activeTab === 'metronome' && (
            <div className="space-y-6">
              {/* Card Central de BPM, Termo de Andamento e Visualizador */}
              <div className="glass-card rounded-3xl p-5 sm:p-6 border border-white/10 bg-slate-900/50 shadow-xl space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5">
                    <button
                      onClick={() => accompanimentStore.tapTempo()}
                      className="px-4 py-2.5 rounded-2xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer active:scale-95 shadow-lg shadow-amber-500/10"
                    >
                  ...
                  <div className="flex items-center justify-center gap-2 sm:gap-3">
                    {Array.from({ length: beatsCount }).map((_, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`flex-1 h-14 max-w-[85px] rounded-2xl flex flex-col items-center justify-center transition-all duration-75 ${ledStyle}`}
                        >
                  ...
                  {accState.hiddenMode && (
                    <div className="mt-2 text-center text-xs font-mono font-bold">
                      {accState.isAudibleMeasure ? (
                        <span className="text-emerald-400">🔊 Compasso com Áudio</span>
                      ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-3xl bg-white/[0.02] border border-white/5 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {accState.muteMetronomeMaster ? 'Metrônomo Mutado' : 'Metrônomo Ativo'}
                    </button>
                  </div>

                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Você pode silenciar o tempo forte ou os fracos individualmente. A contagem de tempo e a banda continuam em sincronia perfeita!
                  </p>
          ...
          {activeTab === 'accompaniment' && (
            <div className="space-y-6">
              ...
              <div className="p-4 sm:p-5 rounded-3xl bg-gradient-to-r from-purple-950/40 via-indigo-950/30 to-black/50 border border-purple-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => accompanimentStore.toggleAccompaniment()}
                    className={`px-4 py-2.5 rounded-2xl font-bold text-xs uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 ${
                      accState.accompanimentEnabled
                        ? 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-600/30 ring-1 ring-purple-400'
                        : 'bg-white/5 border border-white/10 text-slate-400 hover:text-white'
                    }`}
                  >
                    <Disc3 className={`w-4 h-4 ${accState.accompanimentEnabled ? 'animate-spin' : ''}`} />
                    <span>{accState.accompanimentEnabled ? 'Banda Ativa' : 'Banda Desativada'}</span>
                  </button>

                  <div className="text-xs">
                    <span className="text-slate-400">Estilo selecionado: </span>
                    <strong className="text-white">{activeStyle.name}</strong>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 bg-black/50 px-4 py-2 rounded-2xl border border-white/10">
                  <div className="text-right">
                    <span className="text-[10px] font-mono text-purple-300 uppercase block">Compasso {accState.currentMeasure}</span>
                    <span className="text-xs text-slate-400 font-mono">Tempo {accState.currentBeat} de {beatsCount}</span>
                  </div>
                  <div className="w-12 h-12 rounded-xl bg-purple-600/30 border border-purple-500/50 flex flex-col items-center justify-center">
                    <span className="font-display font-black text-white text-lg leading-none">{currentChord?.symbol}</span>
              ...
              <div className="p-5 rounded-3xl bg-white/[0.02] border border-white/5 space-y-4">
                ...
                <div>
                  <span className="text-[11px] font-mono text-slate-400 uppercase font-bold block mb-1.5">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${
                          accState.rootKey === r.note
                            ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-md shadow-purple-600/30'
                            : 'bg-white/5 text-slate-400 hover:text-white hover:bg-white/10'
                        }`}
                      >
                        {r.label}
                      ...
                      return (
                        <button
                          ...
                          className={`p-3 rounded-2xl border text-left transition-all cursor-pointer ${
                            isSelected
                              ? 'bg-purple-600/20 border-purple-500/60 ring-1 ring-purple-400 text-white'
                        ...
                        >
                          ...
                          <div className="font-mono text-xs font-black text-amber-300 mt-1">
                            {prog.romanNumerals}
                          </div>
                      ...
                      return (
                        <div
                          ...
                          className={`px-3 py-1.5 rounded-xl border text-center transition-all ${
                            isNow
                              ? 'bg-amber-400 text-slate-950 font-black border-amber-300 scale-105 shadow-md shadow-amber-400/30'
                              : 'bg-white/5 border-white/5 text-slate-300'
                          }`}
                        >
                          <div className="text-xs font-mono font-bold">{chord.symbol}</div>
          ...
          {activeTab === 'mixer' && (
            <div className="space-y-6">
              ...
              <div className="p-5 sm:p-6 rounded-3xl bg-slate-900/60 border border-white/10 space-y-5 shadow-xl">
                <div>
                  ...
                  {(
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-xs font-bold text-white block mb-2">{ch.label}</span>

                        {/* Slider de Volume da Pista */}
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px] font-mono text-slate-400">
                            <span>Vol</span>
                            <span className="font-bold text-white">{channel.volume}%</span>
                          </div>
                          <input
                            type="range"
                            min="0"
                            ...
                            className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-indigo-500 disabled:opacity-30"
                          />
                        </div>
                      </div>
                    );
              ...
              {/* Balanço Geral: Metrônomo vs Banda */}
              <div className="p-5 rounded-3xl bg-white/[0.02] border border-white/5 space-y-4">
                <span className="text-xs font-mono uppercase font-bold text-slate-300 block">
                  Balanço Geral de Mixagem:
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Master Metrônomo */}
                  <div className="p-4 rounded-2xl bg-black/40 border border-white/5 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-indigo-300">Volume Master do Metrônomo</span>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-mono text-slate-300 hover:text-white border border-white/5 transition-colors cursor-pointer"
                  >
                    🎯 Só Metrônomo
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-indigo-600/30 hover:bg-indigo-600/40 text-xs font-mono text-indigo-200 border border-indigo-500/40 transition-colors cursor-pointer"
                  >
        ...
        {/* 3. Rodapé Persistente com Botão Start / Stop Gigante */}
        <div className="p-4 sm:p-5 border-t border-white/10 bg-slate-950 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div
            ...
            <div className="text-xs font-mono text-slate-400">
              {accState.isPlaying ? (
                <span>
                  Tocando a <strong className="text-white">{accState.bpm} BPM</strong> • Compasso {accState.currentMeasure} • Tempo {accState.currentBeat}
                </span>
              ) : (
                <span>Parado (Ajuste os parâmetros e clique em Iniciar)</span>
              )}
            </div>
          </div>

          <button
            onClick={() => accompanimentStore.toggle()}
