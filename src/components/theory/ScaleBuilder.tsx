...
export const ScaleBuilder: React.FC<ScaleBuilderProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* 2. Painel Unificado & Compacto de Filtros (Família, Escala e Tônica) */}
      <div className="p-4 rounded-2xl bg-black/40 border border-white/5 space-y-3.5">
        {/* Linha 1: Categoria & Escala */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            >
              <span>⭐</span>
              <span>Pentatônicas (4)</span>
            </button>
            <button
              onClick={() => handleSelectCategory('modes')}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1 border ${
                selectedCategory === 'modes'
                  ? 'bg-indigo-600 text-white font-black border-indigo-400 shadow-md shadow-indigo-600/20'
                  : 'bg-white/5 hover:bg-white/10 text-slate-300 border-white/5'
              }`}
            >
              <span>🏛️</span>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1 border ${
                selectedCategory === 'minor'
                  ? 'bg-purple-600 text-white font-black border-purple-400 shadow-md shadow-purple-600/20'
                  : 'bg-white/5 hover:bg-white/10 text-slate-300 border-white/5'
              }`}
            >
              <span>🎻</span>
            ...
            >
              <span>🌌</span>
              <span>Simétricas (2)</span>
            </button>
          </div>

          {/* Escalas da Família Ativa */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] font-mono text-slate-400 uppercase font-bold mr-1">
              Escala:
            </span>
          ...
          {ROOT_KEYS.map((k) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-xl text-xs font-bold font-mono transition-all cursor-pointer border ${
                  isSelected
                    ? 'bg-rose-500 text-white font-black border-rose-400 shadow-md shadow-rose-500/30 ring-1 ring-rose-400'
                    : 'bg-white/5 hover:bg-white/10 text-slate-300 border-white/5'
                }`}
              >
                {k.label}
      ...
      {/* 3. Cards de Tecla das Notas da Escala com Dedilhado & Áudio */}
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-xs font-mono text-slate-300 uppercase tracking-wider font-bold">
            Cards de Tecla em {computed.rootNote} (Toque na Nota para Ouvir):
          </span>
          <span className="text-[11px] text-slate-400 font-mono">
            {computed.notes.length} notas • Dedilhado sugerido para piano
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3">
          {computed.notes.map((noteItem, idx) => {
            ...
            // Cálculo do Dedilhado padrão para piano (Mão Direita: D1 a D5)
            const finger = computed.notes.length === 5
              ? [1, 2, 3, 4, 5][idx % 5]
              : computed.notes.length === 6
              ? [1, 2, 3, 4, 1, 2][idx % 6]
              : [1, 2, 3, 1, 2, 3, 4][idx % 7];
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded-2xl border text-center transition-all cursor-pointer select-none active:scale-95 group relative flex flex-col justify-between min-h-[140px] shadow-lg ${
                  isActive
                    ? 'bg-emerald-500/30 border-emerald-400 ring-2 ring-emerald-400 shadow-emerald-500/25 scale-105 z-10'
                    : noteItem.isTonic
                    ? 'bg-gradient-to-b from-rose-950/40 to-slate-900/60 border-rose-500/40 text-white hover:border-rose-400'
                    : noteItem.isBlueNote
                    ? 'bg-gradient-to-b from-amber-950/40 to-slate-900/60 border-amber-500/50 text-amber-200 hover:border-amber-400'
                    : noteItem.isCharacteristic
                    ? 'bg-gradient-to-b from-cyan-950/40 to-slate-900/60 border-cyan-500/40 text-cyan-200 hover:border-cyan-400'
                    : 'bg-gradient-to-b from-white/[0.07] to-white/[0.02] border-white/10 text-slate-200 hover:border-white/20'
                }`}
                title="Clique para ouvir esta nota no piano"
              >
                {/* Grau da Escala + Som */}
                <div className="flex items-center justify-between text-[11px] font-mono font-bold mb-1">
                  ...
                  <Volume2 className="w-3.5 h-3.5 text-slate-400 group-hover:text-white transition-colors" />
                </div>

                {/* Nome Universal da Nota em Grande Destaque */}
                <div className="text-3xl font-black font-display tracking-tight text-white my-1">
                  {noteItem.note}
                </div>
                ...
                {/* Dedilhado Sugerido (Padrão D1..D5) */}
                <div className="mt-2 pt-1.5 border-t border-white/5 flex items-center justify-between text-[10px] font-mono">
                  <span className="text-slate-400">Dedilhado:</span>
                  <span className="px-1.5 py-0.2 rounded-md bg-indigo-500/20 text-indigo-300 font-bold border border-indigo-500/30">
                    D{finger}
                  </span>
                </div>
      ...
      {/* 4. Laboratório de Avaliação de Performance (com Teclado Integrado) */}
      {showEvaluator ? (
        ...
        /* Standalone keyboard only when evaluator is hidden */
